    ...
    export default {
        ...
        methods: {
			...
			check() {
				...
				if (this.$store.state.form._basic['电子邮箱'] !== null && !patt_email.test(this.$store.state.form._basic['电子邮箱'])) {
					this.warn(jb, '电子邮箱', 'wrong')
					return false
				}

				//单位人才情况汇总校验
				if (!this.checkEveryYear(rchz, '职工人数', this.$store.state.form._summary) ||
				...
				for (var item_out of this.$store.state.form._out_status) {
					...
					if (!this.checkJob(item_out.info['岗位类别'])) {
						this.warn(lcxx + '第' + item_out_index + '项', '岗位类别', 'complete' )
						return false
					}
					item_out_index++
				}
				...
				for (var item_need of this.$store.state.form._need) {
					for (var item_need_s in item_need.info) {
						if (item_need.info[item_need_s] == null && item_need_s != "职业资格证书") {
			...
			checkEveryYear(title, sumname, e) {
				...
				const null_age = '{"30岁以下":null,"31-41":null,"41-50":null,"51-60":null,"61岁以上":null}'
				const null_degree = '{"博士研究生":null,"硕士研究生":null,"本科":null,"大专":null,"大专以下":null}'
				const null_salary =
				...
				const null_award = '{"国家级":null,"省部级":null,"厅局级":null,"其它":null,"无":null}'
							...
							if (info_item == "岗位类别") {
								...
								for (var num_cur of summary_item.info[info_item]) {
									...
									if(!this.checkJob(num_cur.cas)) {
										this.warn(rchz_y + '岗位类别', '第' + job_count + '项', 'complete' )
										return false
									}
									job_count++
								}
